...
function getColorForName(name) {
  var colors = ['#FF5733', '#33B5FF', '#33FF99', '#FF33EC', '#ffcb20', '#9D33FF', '#33FFDA', '#FF8C33', '#3385FF', '#FF3333'];
  var hash = 0;
...
function renderTask(task) {
  var colMap = { todo: '.to-do-tasks', 'in-progress': '.in-progress-tasks', await: '.await-tasks', done: '.done-tasks' };
...
function updateTaskCard(card, task) {
  ...
  for (var i = 0; i < assignedTo.length; i++) {
    ...
    var initials = name.split(" ").map(part => part[0].toUpperCase()).join("");
  ...
  var statusClass = task.status == 'todo' ? 'progress-25' : task.status == 'in-progress' ? 'progress-50' : task.status == 'await' ? 'progress-75' : 'progress-100';
...
function setupDropTargets() {
  var columns = document.querySelectorAll('#board .task-column');
...
function handleDrop(task, column) {
  ...
  var map = { 'to-do-tasks': 'todo', 'in-progress-tasks': 'in-progress', 'await-tasks': 'await', 'done-tasks': 'done' };
  var newStatus = map[column.classList[0]];
  if (card && newStatus) {
...
function updateProgressBar(card, newStatus) {
  ...
  var statusClass = newStatus == 'todo' ? 'progress-25' : newStatus == 'in-progress' ? 'progress-50' : newStatus == 'await' ? 'progress-75' : 'progress-100';
  bar.classList.add(statusClass);
}
...
function initAddTaskOverlay() {
  ...
  for (var k = 0; k < prioButtons.length; k++) {
    prioButtons[k].onclick = function() {
      ...
      document.querySelector('#task-priority').value = this.getAttribute('data-prio');
    };
  }
  ...
  setTimeout(function() {
    if (window.getComputedStyle(addTaskOverlay).display != 'none' && !addTaskOverlay.dataset.userOpened) {
      addTaskOverlay.classList.add('d-none');
      addTaskOverlay.style.display = 'none';
    }
  }, 1000);
