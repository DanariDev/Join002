...
function resetErrors(input, errors) {
  ...
  errors.forEach(e => {
    e.textContent = "";
    e.removeAttribute("data-i18n-error");
  });
}
 ...
 */
function showError(input, errorElem, key) {
  ...
  errorElem.forEach(e => {
    e.textContent = msg;
    e.setAttribute("data-i18n-error", key);
  });
}
...
function nameCheck(id, hasError) {
  ...
  if (!input.value.trim()) { showError(input, error, "common.errors.nameRequired"); hasError = true; }
  return hasError;
}
...
function emailCheck(id, hasError) {
  ...
  if (!value) { showError(input, error, "common.errors.emailRequired"); hasError = true; }
  else if (!/^\S+@\S+\.\S+$/.test(value)) { showError(input, error, "common.errors.emailInvalid"); hasError = true; }
  return hasError;
}
...
function phoneCheck(id, hasError) {
  ...
  resetErrors(input, error);
  const value = input.value.trim();
  if (!value) { showError(input, error, "common.errors.phoneRequired"); hasError = true; }
  else if (!/^[+]?\d+$/.test(value)) { showError(input, error, "common.errors.phoneInvalid"); hasError = true; }
  return hasError;
}
...
function passwordCheck(id, hasError) {
  ...
  resetErrors(input, error);
  const value = input.value.trim();
  if (!value) { showError(input, error, "common.errors.passwordRequired"); hasError = true; }
  else if (value.length < 6) { showError(input, error, "common.errors.passwordShort"); hasError = true; }
  return hasError;
}
...
function passwordRepeatCheck(id, hasError) {
  ...
  if (!value) { showError(input, error, "common.errors.passwordRepeatRequired"); hasError = true; }
  else if (value !== pw) { showError(input, error, "common.errors.passwordMismatch"); hasError = true; }
  return hasError;
}
...
function privacyCheckbox(id, hasError) {
  ...
  if (!checkbox.checked) { showError(checkbox, error, "common.errors.privacyRequired"); hasError = true; }
  return hasError;
}
...
function incorrectDataToAuthentication(emailId, pwId, code) {
  ...
  if (["auth/invalid-email", "auth/user-not-found", "auth/wrong-password", "auth/invalid-credential"].includes(code)) {
    ...
    showError(pwInput, pwError, "common.errors.loginInvalid");
  } else {
    showError(emailInput, emailError, "");
    showError(pwInput, pwError, "common.errors.unexpectedError");
  }
}
